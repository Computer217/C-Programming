...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    close(fdp[1]); //close parent w
    close(fdc[0]); // close child r
    int x,y,z;
    x = 0;
    y = 0;
    z = 1;
    y = gmn_get_max();
    write(fdc[1], &y, sizeof(int));
    do
    {
    	read(fdp[0], &x, sizeof(int));
    	z = gmn_check(&gmn, x);
    	write(fdc[1], &z, sizeof(int));
    } while ( z != 0);
    
    write(fdc[1], gmn_get_message(&gmn), sizeof(char)*strlen(gmn_get_message(&gmn)));
    close(fdp[0]);
    close(fdc[1]);
...
int main(int argc, char *argv[])
{
    ...
    char vehicle_out;
    do{
    	read(fdc[0],&vehicle_out, sizeof(char));
    	printf("%c", vehicle_out);
    }while(vehicle_out!='\n');
    close(fdp[1]);
    close(fdc[0]);
