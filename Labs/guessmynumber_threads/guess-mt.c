...
void * thread_c(void * arg_in)
{
    ...
    do{
    	...
    	arg->guess = arg->guess; //guess
    	arg->result=gmn_check(&gmn, arg->guess); //check guess and send result to parent
    	arg->status = S_RESULT;
    	pthread_cond_signal(&arg->cond_result);
    	
    	pthread_mutex_unlock(&arg->mutex);
    } while (arg->result != 0); // repeat until guessed correctly
    
    //send correct guess to parent 
...
void * thread_p(void *arg_in)
{
    ...
    do { 
        ...
        pthread_mutex_lock(&arg->mutex);
        arg->guess = guess;  //send guess to child
        arg->status = S_GUESS;
...
int main(int argc, char *argv[])
{
    ...
    //thread parent
    int rp = pthread_create(&thread[0], NULL, thread_p, &arg);
    if(rp!=0){
    	perror("pthread_create() failed");
    	exit(0);
    }
    //thread child
    int rc = pthread_create(&thread[1], NULL, thread_c, &arg);
    if(rc!=0){
    	perror("pthread_create() failed");
    	exit(0);
    }
