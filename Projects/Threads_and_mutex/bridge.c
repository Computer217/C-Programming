...
void cross_bridge_single(car_t * car)
{
    ...
    omly waits if it cant cross the bridge 
...
//function takes a pointer to the car structure 
int check_west(car_t * car){
	//initialize bridge structure and make it same bridge that the car is going to use 
	bridge_t* car_bridge = car->bridge;
	//check if directions align westbound 
	if(car->direction == WEST && car_bridge->direction == WEST) {
        if(car_bridge->n_crossed + car_bridge->n_crossing < car_bridge->max_crossed && car_bridge->n_crossing < car_bridge->max_crossing) {
            return 1;
        }
        //not waiting to go east and number of cars crossing isnt maxed out
        else if(car_bridge->eastb_waiting == 0 && car_bridge->n_crossing < car_bridge->max_crossing) {
            return 1;
        }
        else{
        	return 0; //check
    ...
    //if directions dont align but there are no cars crossing
    else if(car_bridge->direction == EAST && car->direction== WEST && car_bridge->n_crossing==0) {
    	//if its not waiting to go east or current crossed cars is greater than or equal to max allowed
        if(car_bridge->eastb_waiting==0 || car_bridge->n_crossed >= car_bridge->max_crossed) {
            return 1; //car is valid to go other direction 
        }
    }
...
int check_east(car_t * car){
	bridge_t* car_bridge = car->bridge;
	//CHECK THAT THE DIRECTION OF THE CAR IS RIGHT DIRECTION THAT BRIDGE IS ALLOWING TRAVEL
	if(car->direction == EAST && car_bridge->direction == EAST){
		//check if its posssible to cross based on max_crossed
		if (car_bridge->n_crossed +car_bridge->n_crossing < car_bridge->max_crossed && car_bridge->n_crossing < car_bridge->max_crossing){
			return 1; // valid to cross on 1 return 
		}
		else if (car_bridge->westb_waiting == 0 && car_bridge->n_crossing < car_bridge->max_crossing){ //check
			return 1;
		}
		else{
			return 0; //check
	...
	else if(car->direction == EAST && car_bridge->direction== WEST && car_bridge->n_crossing==0){
        if(car_bridge->westb_waiting==0 || car_bridge->n_crossed >= car_bridge->max_crossed) {
            return 1;
        }
        else{
        	return 0; //check
