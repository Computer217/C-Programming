...
{
    // TODO
    printer_t * print = (printer_t *)arg;
    print->njobs = 0;
    
    int next = q_num_jobs(print->jq); //whats next in terms of jobs
    while(next){
    	pthread_mutex_lock(&print->jq->mutex);
    	int fetch_it = q_fetch_job(print->jq, print->id); //fetch a job 
    	
    	pthread_mutex_unlock(&print->jq->mutex); 
    	
    	print_job(fetch_it); //print 
    	print->njobs += 1; 
    	next = q_num_jobs(print->jq);
    }
    return arg;
}
...
int main(int argc, char *argv[])
{
    ...
    if (demo) {
    	...
    	for (int i=0; i<num_printers; i++){
    		...
    		int r= pthread_create(&thread[i], NULL, printer_main, &printers[i]);
    		if(r){
    			exit(1);
    		}
    	}
    	for(int j=0; j<num_printers; j++){
    		int r= pthread_join(thread[j], NULL);
    		if(r){
    			exit(1);
    		}
    	}
